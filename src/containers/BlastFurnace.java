...
public class BlastFurnace extends ContainerProcessors{
	...
	public BlastFurnace(Main_Game game, short id, int chunkX, int chunkY, int tileX, int tileY) {
		super(game, id, ContainerIDs.BlastFurnace, validSlots, "Files/File "+game.CurrentFile+"/Tiles/BlastFurnace "+id+".txt", neededValues, chunkX, chunkY, tileX, tileY);
		this.game = game;
		for (int i = 0; i < neededValues; i++) {
			getValues()[i] = 0;
		}
		getValues()[1] = Main_Game.BlastFurnaceCookTime;
	}
	...
	public void tick() {
		if (getValues()[0] == 1) { //If the blast furnace is running
			...
			values[1] = Main_Game.BlastFurnaceCookTime;
		}
	}
	...
	public void render(Graphics g) {
		if (getValues()[0] == 1) {
			...
			g.drawImage(game.tileanimations.animationImage, x+(game.TileWidth*loc[2])-(game.TileX*game.TileWidth)-(game.TileWidth*16)-game.x+((loc[0]-game.ChunkX)*game.TileWidth*16), y+(game.TileHeight*loc[3])+(game.TileY*game.TileHeight)-(game.TileHeight*16)+game.y+((loc[1]-game.ChunkY)*game.TileHeight*16), x+(game.TileWidth*(loc[2]+1))-(game.TileX*game.TileWidth)-(game.TileWidth*16)-game.x+((loc[0]-game.ChunkX)*game.TileWidth*16), y+(game.TileHeight*(loc[3]+1))+(game.TileY*game.TileHeight)-(game.TileHeight*16)+game.y+((loc[1]-game.ChunkY)*game.TileHeight*16), 0, 0, 320, 320, null);
		}
	}
}
