...
public enum ContainerIDs {
	...
	ContainerIDs(SurfaceTileIDs tileID, boolean electronic, boolean onOff, boolean hasFlame) {
		this.tileID = tileID;
	...
	public static boolean containerExists(String containerName) {
		for (int i = 0; i < ContainerIDs.values().length; i++) {
			if (ContainerIDs.values()[i].toString() == containerName) {
				return true;
			}
		}
		return false;
